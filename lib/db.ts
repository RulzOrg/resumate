...
import { normalizeSalaryRange } from "./normalizers"
...
export async function createJobAnalysisWithVerification(data: {
  ...
  try {
    ...
    const normalizedAnalysisResult = {
      ...
      salary_range: normalizeSalaryRange(data.analysis_result.salary_range),
    }

    const keywords = Array.isArray(normalizedAnalysisResult.keywords) ? normalizedAnalysisResult.keywords : []
    const required_skills = Array.isArray(normalizedAnalysisResult.required_skills) ? normalizedAnalysisResult.required_skills : []
    const preferred_skills = Array.isArray(normalizedAnalysisResult.preferred_skills) ? normalizedAnalysisResult.preferred_skills : []
    const experience_level = normalizedAnalysisResult.experience_level || null
    const salary_range = normalizedAnalysisResult.salary_range || null
    const location = normalizedAnalysisResult.location || null
    ...
    const [analysis] = await sql`
      ...
      VALUES (
        ...
        ${data.job_url || null}, ${data.job_description}, ${JSON.stringify(normalizedAnalysisResult)},
