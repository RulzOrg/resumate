  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-1">
        <div className="sticky top-8 space-y-4">
          ...
          <div className="p-5 rounded-xl border border-border bg-card backdrop-blur-sm">
            <h2 className="text-sm font-medium text-center mb-4 text-muted-foreground uppercase tracking-wider font-sans">
              Your Score
            </h2>
            ...
            <div className="relative w-44 h-24 mx-auto mb-3">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-end pb-1">
                <span className={cn(
                  ...
                  {results.overallScore}<span className="text-lg text-muted-foreground/70">/100</span>
                </span>
            ...
            <button
              ...
              className="w-full mt-5 py-3 px-4 rounded-full font-medium text-sm transition-all bg-primary text-primary-foreground hover:bg-primary/90 flex items-center justify-center gap-2 font-sans shadow-lg shadow-primary/20"
            >
          ...
          <div className="p-4 rounded-xl border border-border bg-card backdrop-blur-sm">
            <h3 className="font-medium text-sm mb-3 font-sans flex items-center gap-2 text-foreground/80">
              <TrendingUp className="w-4 h-4 text-muted-foreground/70" />
              Quick Summary
            </h3>
            {results.summary.strengths.length > 0 && (
              <div className="mb-3">
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1.5 font-sans">Strengths</p>
                <ul className="space-y-1.5">
                  {results.summary.strengths.slice(0, 3).map((s, i) => (
                    <li key={i} className="text-xs flex items-start gap-2 text-foreground/80 font-sans">
            ...
            {results.summary.criticalIssues.length > 0 && (
              <div>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1.5 font-sans">Top Issues</p>
                <ul className="space-y-1.5">
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        {results.tailoring ? (
          ...
          <div className="p-5 rounded-xl border border-border bg-card">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-sm font-medium uppercase tracking-wide font-sans text-foreground/80">TAILORING</span>
              <span className="px-2 py-0.5 rounded-full text-[10px] bg-muted text-muted-foreground/70 font-sans">
                Not Available
              </span>
            </div>
        ...
        {/* Bottom CTA */}
        <div className="p-5 rounded-xl border border-primary/20 bg-gradient-to-r from-primary/5 via-primary/5 to-transparent">
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            <div className="flex-1">
              <h3 className="font-semibold text-base mb-1 font-sans text-foreground">Ready to fix these issues?</h3>
              <p className="text-sm text-muted-foreground font-sans">
            ...
            <button
              onClick={onCreateAccount}
              className="px-5 py-2.5 rounded-full font-medium text-sm transition-all whitespace-nowrap bg-primary text-primary-foreground hover:bg-primary/90 flex items-center gap-2 font-sans shadow-lg shadow-primary/20"
            >
              Create Free Account
              <ArrowRight className="w-4 h-4" />
            </button>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span className="text-[11px] font-medium uppercase tracking-wide text-foreground/80">{name}</span>
        {issueCount !== undefined && issueCount > 0 && (
          <span className="px-1.5 py-0.5 rounded-full text-[10px] bg-muted text-muted-foreground/70">
            {issueCount}
          </span>
        )}
      </div>
...
function CategorySection({
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden backdrop-blur-sm">
      ...
      <button
        onClick={onToggle}
        className="w-full flex items-center justify-between p-4 hover:bg-accent transition-all duration-200"
      >
        <div className="flex items-center gap-3">
          <span className="font-medium text-sm font-sans text-foreground">{title}</span>
          {issueCount > 0 && (
            <span className="px-2 py-0.5 rounded-full text-[10px] bg-muted text-muted-foreground/70 font-sans">
        ...
        <div className="border-t border-border p-4 space-y-4">
          ...
          <div className="space-y-2">
            {subcategories.map((sub, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-3 rounded-lg bg-white dark:bg-card border border-border"
              >
                <div className="flex items-center gap-2">
                  {sub.status === "pass" ? (
                  ...
                  <span className="text-sm font-medium text-foreground/80 font-sans">{sub.name}</span>
                  {sub.issues.length > 0 && (
                    <span className="px-1.5 py-0.5 rounded-full text-[10px] bg-muted text-muted-foreground/70 font-sans">
                      {sub.issues.length} issue{sub.issues.length > 1 ? "s" : ""}
                    </span>
                  )}
                </div>
                {sub.details && (
                  <span className="text-xs text-muted-foreground/70 font-sans">{sub.details}</span>
                )}
              </div>
            ))}
          </div>
          ...
          {issues.length > 0 && (
            <div className="space-y-3 pt-2">
              <h4 className="text-xs font-medium text-muted-foreground/70 uppercase tracking-wider font-sans">Issues</h4>
              {issues.map((issue) => (
    ...
    <div className={cn(
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <h5 className="font-medium text-sm text-foreground/90 font-sans">{issue.title}</h5>
            <p className="text-xs text-muted-foreground mt-1 font-sans leading-relaxed">{issue.description}</p>
          </div>
        </div>
        <button
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-border space-y-3">
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1.5 font-sans">
              Recommendation
            </p>
            <p className="text-sm text-foreground/80 font-sans leading-relaxed">{issue.recommendation}</p>
          </div>

          {issue.fixable && (
            <button
              onClick={() => onFix(issue.id)}
              className="px-4 py-2 rounded-full text-xs font-medium transition-all bg-primary text-primary-foreground hover:bg-primary/90 font-sans shadow-lg shadow-primary/20"
            >
              Fix This Issue
            </button>
          )}
