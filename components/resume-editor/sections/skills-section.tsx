...
export function SkillsSection() {
  const { state, resumeId, updateSkill, addSkill, removeSkill } = useEditor()
  const { skills } = state
      ...
      const response = await fetch(`/api/resumes/${resumeId}/enhance`, {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            currentSkills: skills.map(s => s.value)
          }
        })
      })
    ...
    <SectionWrapper title="Skills" onEnhance={handleGenerate}>
      <div className="relative">
        ...
        <p className="text-sm text-neutral-400">
          Click a skill to toggle inclusion. Hover to remove.
        </p>
            ...
            <div key={skill.id} className="group relative">
              <label className="inline-block cursor-pointer">
                <input
                  ...
                  onChange={(e) => updateSkill(skill.id, 'include', e.target.checked)}
