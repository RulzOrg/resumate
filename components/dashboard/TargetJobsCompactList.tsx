  ...
  return (
    <div className="space-y-4">
      {items.map((job) => (
        <div
          key={job.id}
          className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-lg bg-surface-subtle dark:bg-white/5 border border-border dark:border-white/10"
        >
          <div className="flex items-center gap-4 flex-1">
            ...
            <div>
              ...
              <p className="text-sm text-foreground/60 dark:text-white/60">{job.company_name || "Company"}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 self-start sm:self-center">
            <Link href={`/dashboard/jobs/${job.id}`} className="text-xs font-medium text-foreground/70 dark:text-white/70 hover:text-foreground dark:hover:text-white transition">
              View
            </Link>
            <Link
